...
\end{align}
where $G$ is the gravitational constant, $P$ is pressure, $\rho$ is density and $K$, $\Lambda$ and $a$ are cosmology-related parameters \citep{mo2010galaxy}. $K$ specifies the curvature of the universe, determined by the overall density of the universe \citep{mo2010galaxy}. The allowed values being for $K$ are $-1$ corresponding to a hyperbolic universe, $0$ to a flat one and $1$ to a spherical universe \citep{mo2010galaxy}. Modern measurements suggest that the Universe is flat within the measurement error, but small deviations from the $K=0$ cannot be ruled out \citep{planck2016resultsI}. $\Lambda$ is the cosmological constant driving the expansion of the universe, often described as dark energy or vacuum energy \citep{mo2010galaxy}.

The most relevant for this thesis of these cosmological parameters is the scale factor $a$. It increases or decreases as the universe expands or contracts, and thus it can be used to relate distances at different times \citep{mo2010galaxy}. For universe that is monotonically expanding the scale factor can also be used as an alternative time coordinate, as it has one-to-one correspondence to time. It is often convenient to measure not the proper distance $l$ between a pair of objects, but instead so-called comoving distance $r$: \citep{mo2010galaxy}
...
Using the Friedmann equations \ref{friedmann1} and \ref{friedmann2} to model the evolution of the Universe requires not only the equations and cosmological parameters introduced in the section \ref{universe-evolution}, but also the composition of the universe has to be known in order to determine the density, pressure, cosmological constant and ultimately even the curvature of the universe. According to the current understanding, the universe is made of three components: non-relativistic matter, relativistic matter and vacuum energy \citep{mo2010galaxy}. At present time, about 69 \% of the energy density in the universe is vacuum energy and 31 \% is non-relativistic matter, including both baryonic and dark components \citep{planck2016resultsI}. Most of the matter, around 84 \%, is dark matter, with baryonic matter only contributing around 16 \% of the total matter energy density \citep{planck2016resultsI}. Energy density of relativistic matter such as photons and neutrinos in the modern universe is negligible \citep{mo2010galaxy}.

This has not always been true, as these ratios change over time as the Universe evolves. It is easy to see that as the Universe expands, energy density of matter behaves as $a^{-3}$, as the volume of the volume of the Universe increases as $a^3$ and in adiabatic system no new matter is created or disappears. Radiation is diluted similarly to matter, but in addition to the effect of increasing volume, the growing universe also causes the wavelength of the radiation lengthening, thus resulting in the energy density decreasing as $a^{-4}$. The vacuum energy, as the name suggests, can be thought as arising from the space itself, and thus change in $a$ does not affect the energy density of the component \citep{mo2010galaxy}. The change of energy density of a component results in the pressure and temperature of the component changing too, these effects are shown in table \ref{tab:thermodynamics} \citep{mo2010galaxy}.
...
The differences in time evolutions of the components also mean that the dominant component of the Universe changes as the scale factor grows. At very early times the Universe was radiation dominated, but as the radiation energy density decreases faster than the energy densities of the two other components, a matter dominated era will follow. As the dark energy is the only component with constant energy density, it will be the final dominant energy component. This transition from matter to dark energy dominated era has either happened in the recent past or will happen in the near future \citep{mo2010galaxy}.

The geometry of the Universe is also determined by the contents of it. A density threshold known as critical density and defined as
\begin{equation}
\rho_{crit,0} \equiv \frac{2H_0^2}{8\pi G}
\end{equation}
acts as a threshold value that separates the different geometries \citep{mo2010galaxy}. The overall density of the universe can be parametrized using this critical density:
...
\end{equation}
where $m$, $r$ and $\Lambda$ stand for non-relativistic matter, relativistic matter and vacuum energy \citep{mo2010galaxy}. Each of these can be calculated similarly to the equation \ref{Omega} but replacing the overall density with density of the corresponding component \citep{mo2010galaxy}. As in the Universe the value of $\Omega_0$ is very close to unity, the different density parameters conveniently correspond the relative densities of the components.


\subsection{Structure Formation} \label{universe-structure}
All structures in the Universe arise from the small density perturbations in the early universe and the physics that govern their evolution. These primordial density fluctuations that later develop to the structures such as galaxies and galaxy clusters can still be seen in the cosmic microwave background (CMB) \citep{planck2016resultsI}. At $z \approx 1100$, the time of recombination and origin of the CMB photons, these fluctuations are still very small, but sufficiently overdense regions are able to collapse in on themselves \citep{mo2010galaxy}. This requires the volume to be sufficiently dense to allow gravity to overcome the expansion of the universe as described by the Friedmann equations.

To understand the evolution of density perturbations at early times when the perturbations are still small and evolve linearly, let us consider an ideal fluid of density $\rho$, moving at velocity $\mathbf{u}$ and experiencing the gravitational field with potential $\phi$. Growth of a perturbation in this medium is governed by three equations: the equation of continuity describing the conservation of mass, Euler equations governing the motions in the fluid and the Poisson equation describing the gravitational field, or
\begin{equation}
\frac{\textrm{D}\rho}{\textrm{D}t} + \rho \nabla_r\dot{\mathbf{u}} = 0,
\end{equation}
\begin{equation}
\frac{\textrm{D}\mathbf{u}}{\textrm{D}t} = - \frac{\nabla_r P}{\rho} - \nabla_r\phi
\end{equation}
and
\begin{equation}
\nabla_r^2\phi = 4\pi G\rho
\end{equation}
respectively \citep{mo2010galaxy}. Here $r$ denotes proper coordinates and $\frac{\textrm{D}}{\textrm{D}t}$ is the convective time derivative, defined as
\begin{equation}
\frac{\textrm{D}}{\textrm{D}t} = \pdv{}{t} + \mathbf{u}\dot\nabla_r
\end{equation}
and describing the time derivative when moving with the fluid \citep{mo2010galaxy}.

As the Universe is expanding, it is natural to use comoving coordinates, defined in \ref{comoving} but denoted by $\mathbf{x}$ here. This also affects the velocities, as using the dot to denote time derivative we can write
\begin{equation}
\mathbf{u} = \dot{\mathbf{r}} = \dot{a}(t)\mathbf{x} + a(t)\dot{\mathbf{x}} = \dot{a}(t)\mathbf{x} + \mathbf{v}.
\end{equation}
Here $\mathbf{v}$ denotes the peculiar velocity. The overdensities in the Universe are measured using a density perturbation contrast $\delta$, defined by $\rho(\mathbf{x}, t) = \bar \rho(t)[1+\delta(\mathbf{x}, t)]$ where the bar denotes global average \citep{mo2010galaxy}. Applying these coordinate transforms and reading from table \ref{tab:thermodynamics} that $\rho \propto a^{-3}$, we can rewrite the continuity, Euler and Poisson equations:
*TODO add equations*

% start with LCDM here?
...
%\\- - Initial perturbations (we can already see non-homogeneities in the CMB, spectrum of perturbations), highlight that, at z=300,000, perturbations are still very small ($10^{-4}$).
%\\- - evolution of perturbations: competition between expansion (Friedmann equations discussed earlier, and collapse)
%\\- - Newtonian is fine, discuss collapse of a spherical overdensity (top hat, 1D), including concept of turn-around, turn-around time.
%\\- - caveats: spherical, top-hat, collisionless, zero-angular momentum, no substructures and not mergers, end-point is not physical...
%\\- - - what separates DM, baryons radiation, Lambda (time evolution is different, and baryons experience pressure, and can radiate photons)
%\\- - Concept of a halo: collapsed object, with a boundary set by some overdensity threshold (e.g. r200 defined as the radius within which the overdensity is 200).
%\\- LCDM
%\\- - Model which describes constitutents of the universe (out of many possibilities): key assumptions are: universe contains ordinary matter, radiation, dark matter and dark energy. Evidence for, and properties of, dark matter and dark energy: structure formation agrees (without DM, no BAO, density close to critical (see review on open problems - from SNe and CMB we can constrain Omega\_matter and Omega\_lambda - result is Omega\_L ~ 0.7 and Omega\_m ~ 0.3). 
%\\- - There are also many alternatives to "Lambda", but these affect the "global" evolution of the universe, and are best tested on very large scales.
%In the Local Group, CDM can be tested.
%\\- - Cold: if DM was "hot", thermal motions would wash out small scale perturbations, no structures could form below a certain scale (at least not from primordial fluctuations). This would change the amount of substructures of halos, or of small halos. Here, CDM and WDM differ: for a given halo mass, CDM and WDM predict a different amount of subhalos. We can count the number of subhalos by counting the number of dwarf galaxies (only in the LG can we see them all). Whether or not that number is consistent with CDM or with WDM then depends on the total mass. That's why the MW, M31, and LG mass are so important.
