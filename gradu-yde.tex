...
The Local Group analogues analyzed in this project do not differ only in the properties of the primaries but they also reside in different surroundings. The primaries can for example have few or many satellites around them and the velocity dispersion within the system can vary. Similarly the environment further from the system can be densely populated with haloes arranged in filaments or walls, or the Local Group analogue can be located in a very low density environment.

\begin{figure}
    \centering
    %\def\svgwidth{\columnwidth}
    \includegraphics{kuvat/hubblediagrams.pdf}
    \caption{Hubble Flows around Milky Way in four simulations. TODO: caption %TODO
    }\label{fig:hubblediagrams}
\end{figure}

This is illustrated in Fig.~\ref{fig:hubblediagrams} showing radial velocities as seen from the lower mass primary are plotted against the distance of the subhalo to show what a Hubble diagram would look like as seen from the Milky Way analogue of that simulation. Each of the four panels contains all subhaloes in that simulation up to the closest type 2 particle. The simulations shown in this plot are chosen such that they show a variety of different Hubble Flows, but naturally four simulations can never show the full range of all 199 analyzed simulations let alone all structures that can occur.

The simulations on the top row in Fig.~\ref{fig:hubblediagrams} both show fairly small dispersion of radial velocities on each distance, corresponding to a cold Hubble flow. In the upper left plot, there is a region with much larger velocity dispersion between approximately 4 and 5~Mpc, likely corresponding to another system containing a system satellite galaxies, some of which are receding and some approaching the observer when they orbit the primary or primaries of the system. The upper right plot on the other hand shows no such variation in the velocity dispersion, apart from the dispersion within the nearest 1~Mpc or so being higher than the in rest of the plot. The upper left plot on the other hand shows fairly low velocity dispersion within the Local Group.
