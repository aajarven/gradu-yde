...
The simulations contained only dark matter, and for my analysis only the dark matter halo information was needed, so instead of using the full simulation output the analysis was conducted on subhalo catalogues. The subhalo information was extracted as described in section \ref{sect:halofinding}. The resulting data set consists of subhalo catalogues from 448 zooms, each centered on one region resembling the Local Group in the low-resolution simulation. The following sections shortly introduce the code used to run the simulation and the parameters of the simulation for both stages of the zoom-in simulations.
...
The simulation code that was used to run the simulations on which all of the analysis in this master's thesis is based on was a modified version of GADGET-3, an earlier version of which is described by \citet{springel2005cosmological}. The code is same as used in the EAGLE project, so detailed descriptions of the changes can be found in \citep{schaye2015eagle}. However, the changes mostly affect the handling of baryonic matter so understanding the basic GADGET-2 gives a good basis for understanding the simulations \citep{schaye2015eagle}.
...
First step of a zoom-in simulation running process is to construct initial conditions and run a low-resolution box. A large box with a side length of 542.16 Mpc/h was used to ensure that the box is big enough to contain a reasonable sample of Local Group analogues and that the large scale structure of the universe is represented in the simulation. At $z = 0$ this corresponds to a volume of $800^3$~Mpc$^3$.

The volume contained about $1.3 \times 10^{11}$ dark matter particles, each with a mass of about $1.6 \times 10^8~\mathrm{M_{\astrosun}}$. This determines the mass resolution of the simulation: objects smaller than the mass of a single particle are not seen at all and reliable analysis cannot be made of objects that are made of a small number of particles. This means that for example distribution of spiral and elliptical galaxies can be studied but all dwarf galaxies are not resolved. The spatial resolution of the simulation is determined by the softening length: the distance within the gravitational interaction calculations are modified in order to remove singularities and avoid extreme accelerations. The properties of the parent simulation are shown in table \ref{tab:parentBox}.

In the parent simulation a softening length of 2.3~kpc/$h$ was used, $h$ being the reduced Hubble parameter defined in equation \ref{reducedhubble}. As $h$ evolves with the scale factor, the softening length grows with the simulation box. For regions expanding with the simulated universe this is advantageous, but for e.g. virialized structures with no spatial growth this is an unwanted behaviour. For simulations aimed for studying objects with specific mass and thus specific redshift when they collapse, it is possible to specify a redshift after which the softening length is kept constant in physical units, but for simulations exploring a range of objects this is not possible.

\begin{table}
	\centering
	\begin{tabular}{| l | l |}
		\hline
		Box size & 542.16 Mpc/h \\ \hline
...
The cosmology of the simulation is a standard $\Lambda$CDM cosmology with most important cosmological parameters of \citet{planck2014resultsXVI}. Most important parameters are shown in table \ref{tab:cosmopars}. For a dark matter only simulation the $\Omega_b$ is naturally ignored and all matter treated as dark matter. The $\sigma_8$ parameter shown in the table is used to define the amplitude of the power spectrum in the initial conditions.
...
The low-resolution simulation was then run and when it reached redshift $z=0$, haloes were identified using the friends-of-friends and SUBFIND algorithms as  described in section \ref{sect:halofinding}. A lower limit of 20 particles in each subhalo was used and smaller structures ignored. From the resulting subhalo catalogue, halo pairs resembling the Local Group were identified. TODO criteria %TODO how
For each of these Local Group analogues a zoom simulation was created.


\subsection{Zoom simulations}
The zoom simulations simulated the same box as the parent simulation, but new initial conditions with enhanced resolution around Local Group analogues were created for each pair of objects. The large scale structure and evolution in these zooms is similar to the parent simulation, but due to increased resolution the interesting regions in the vicinity of the Local Group analogues can be studied in more detail. The change of resolution was implemented by dividing the dark matter particles to three categories ranging from the least massive type 1 particles to the most massive type 3 particles corresponding to the mass resolution of the parent simulation. The GADGET-3 simulation code is also capable of handling baryonic matter such as gas and stars, but the simulations used in this thesis were dark matter only simulations.

The type 1 particles were assigned to regions identified to end up in or pass by the central Local Group analogue and a buffer zone of type 2 particles was created around the type 1 particles to reduce the probability of interactions between different types of particles resulting in the lighter particle being subjected to an unrealistically high acceleration. Number and mass of type 1 particles in a simulation varies based on the extent and density of the regions forming the Local Group analogue. Histogram showing the numbers of high resolution particles in each simulation is presented in figure \ref{fig:type1hist}. Number of type 1 particles ranges from about $1.6 \times 10^6$  to nearly $30 \times 10^6$ with lower values being most typical: most of the simulations have less than $7 \times 10^6$ high resolution particles. %1,618,600 to 29,791,000

\begin{figure}
    \centering
    \input{kuvat/tex/type1particles.pdf_tex}
    \caption{Font ok, TODO: caption}\label{fig:type1hist}
\end{figure}


While number and mass of type 1 particles varied slightly from simulation to simulation, every simulation had the same softening length of 1.0~kpc/$h$.


At $z=0$ a subhalo catalogue was created for each of the zoom simulations. This was done using a combination of friends-of-friends ans SUBFIND algorithms similarly to with the parent simulation. Instead of the full particle data, all analysis conducted for this thesis was done using these subhalo catalogues. Initially there were 448 of these catalogues in the data set. Simulations were run and halo catalogues generated by Till Sawala but all further analysis presented is done as a part of this thesis.

First, Local Group analogues were identified in all of the catalogues. This was done by identifying pairs of haloes that are made of type 1 particles and fulfil the following criteria:
\begin{itemize}
...
\item The haloes are moving towards each other with radial velocity no larger than 150 km/s.
\item Tangential velocity of the haloes is smaller than 50~km/s.
\item The two haloes are the most massive ones within 2~Mpc.
\end{itemize}
The criteria was chosen so that the systems resemble the Local Group but the ranges are wide enough to accommodate uncertainties in the observed values and to produce a large enough data set for statistical analysis.

Unfortunately, the 448 simulation runs did not easily translate to 448 Local Group analogues. First of all, some resimulations did not contain a pair of haloes fulfilling the Local Group criteria, which reduced the sample to 354 simulations. Some of the simulations also contained more than one Local Group analogue, in which case the pair with largest distance to the nearest type 2 or 3 particle was chosen for analysis.

The sample size was still further reduced from the 354 Local Group analogues as picking the Local Group analogues from the parent simulation had resulted in many (all?) %TODO
pairs of haloes being picked twice. Two zoom simulations were then run for each of the pairs resulting essentially the same simulation being run twice, providing no new information. Removing one of each of these pairs from the data set resulted in a total of 159 unique Local Group analogues.

As the analysis conducted for this thesis uses not only the properties of a Local Group analogue but also its surroundings, all of the analogues were also required not to have any type 2 or 3 particles within \_ Mpc of the mass centre of the halo pair. %TODO distance
This resulted in a final sample of \_ subhalo catalogues. %TODO number

TODO:\\
- plot uncontaminated distances?\\
- 
