...
The simulations presented in the bottom row of the figure show considerably higher velocity dispersions. Especially the bottom right plot with radial velocities easily spanning a range of 400~km/s on all distances except within 1~Mpc of the Milky Way analogue differs from the top row plots. The bottom right plot also has much greater number of subhaloes than the other plots. The bottom left plot on the other hand seems to show two a Hubble flow that is separated to two parts with a vertical offset of around 100~km/s. This kind of situation might occur for example if the Local Group analogue is moving relative to the surrounding large scale structure or it is located near an intersection of filaments.
...
When not otherwise stated, in this work the number of nearest subhaloes to be ignored is chosen based on the sum of squares of the residuals. This is done by first calculating the ordinary least squares fit using all of the available data up to the first type 2 particle. The mean squared residual per data point used for this fit is then stored together with the fitting result. Then the fitting is repeated, but the subhalo closest to the origin is omitted from the fitting. The squared residuals of the two fits are compared, and the fit with smaller value per data point is stored. Next the two closest subhaloes are omitted, a line is fitted and the better fit stored. This is repeated until all haloes up to 2~Mpc are omitted, at which point the fit with the smallest residuals is chosen as the one that best describes the Hubble flow.
...
This approach is naturally not without weaknesses. If the innermost subhaloes within Local Group for example would have a very small radial velocity dispersion, no subhaloes might be excluded as data points with small dispersion reduce the mean squared residual. Similarly groups of data points with large scatter can get excluded even if they the Hubble Flow neatly. Luckily these problems are usually not severe as even when the maximum of 2 Mpc of the nearest region is excluded from fitting, most simulations still have more than 3~Mpc worth of data to use for fitting and thus small changes in including or excluding subhaloes in the innermost regions do not produce large effects.

The fitting is also repeated multiple times for each simulation, making the fitting slow compared to e.g. the case with a constant exclusion threshold. Adaptive solutions with possibly better time complexity also exist, as one might for example calculate the fit iteratively by always excluding the haloes that are closer than the distance at which the fitted radial velocity is zero. This is an appealing approach as instead of just the scatter around the fit it is based on the physical distance at which the Hubble flow can be said to start. Unfortunately the solution does not necessarily converge but can instead oscillate between two or more solutions and for example in situation shown in Fig.~\ref{fig:hubblefit} the range starting at the point where the fitted radial velocity is zero would likely contain a substantial number of subhaloes with large velocity dispersion that are likely to be bound to the Local Group. Thus the slower method comparing the mean squared residuals is used in this work.
