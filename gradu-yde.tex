...
\begin{equation}
\frac{\textrm{D}}{\textrm{D}t} = \pdv{}{t} + \mathbf{u}\cdot\nabla_r
\end{equation}
and describing the time derivative when moving with the fluid \citep{mo2010galaxy}.

As the Universe is expanding, it is natural to use comoving coordinates, defined in \ref{comoving} and denoted by $\mathbf{x}$ here. This also affects the velocities, and using the dot to denote time derivative we can write
\begin{equation}
