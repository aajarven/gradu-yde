...
Regression analysis is a set of statistical analysis processes that are used to estimate functional relationships between a response variable (often denoted with $y$) and one or more predictor variables (often denoted $x$ or $x_1 \dots x_i$) \citep{feigelson2012modern}. % soossi merkinnälle
One of the models used is linear regression model, which can be used to fit any relationship where response variable is a linear function of the model parameters \citep{montgomery2012introduction}. Widely known and used simple linear regression 
\begin{equation}
	y = \beta_0 + \beta_1 x + \varepsilon
\end{equation}
is one of the linear regression models, but also 
