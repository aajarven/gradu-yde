...
The first step in quantifying the properties of the Hubble flows around the Local Group analogues is to determine the basic shape of the flow. In this work, the Radial velocity field is modelled to consist of two parts: a nearly constant or decreasing radial velocity with possibly large scatter within and near the Local Group analogue and a linear relationship between distance and radial velocity on larger distances. Naturally this is not an accurate description of the situation, as the transition from gravitational collapse within the Local Group to a linear Hubble flow is not instantaneous and the gravitational pull of the Local Group also affects objects outside it.


TODO: In this work, this is done using ordinary least squares regression (see section \ref{sect:simple-linear}) %TODO
