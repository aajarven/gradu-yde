...
Simulations are a valuable tool in astrophysical research as they bypass many major restrictions of observational astronomy such as not being able to observe the evolution of a single object with the exception of the most rapid events such as supernovae, and constraining the observations to a single view plane.
...
\begin{figure}
    ...
    \caption{Cells of figure %\ref{fig:tree-box} 
    shown as a tree, each level of refined cells in %\ref{fig:tree-box} 
    traversed from left to right and up to down corresponding to each level of nodes from left to right. Cells with no particles are not used in force calculations and thus they do not need to be stored in computer memory, but they are shown here to emphasize the structure of the tree.}\label{fig:tree}
\end{figure}
