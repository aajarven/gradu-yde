...
[insert some form of attribution for the simulations, e.g. citation if available]. The simulations are cosmological zoom-in simulations, meaning that a higher resolution was used in interesting regions. For these simulations, this was done by identifying interesting regions from an output of a low-resolution simulation at $z=0$, after which resolution of the interesting volumes was increased and these new simulations were run. In this case, the interesting regions were defined as surroundings of a pair of dark matter haloes resembling ones of the Milky Way and Andromeda galaxies, the two main galaxies of the Local Group.
...
The first step in this kind of a zoom-in simulation running process is to construct initial conditions and run a low-resolution box. A large box with a side length of 542.16 Mpc/$h$, $h$ being the reduced Hubble parameter defined in equation \ref{reducedhubble}, was used to ensure that the box is big enough to contain a reasonable sample of Local Group analogues and that the structure of the universe is represented up to reasonably large scales in the simulation. At $z = 0$ the simulation volume is $800^3$~Mpc$^3$.
...
In the parent simulation a softening length of 2.3~kpc/$h$ was used. As $h$ evolves with the scale factor, the softening length grows with the simulation box. For regions expanding with the simulated universe this is advantageous, but for e.g. virialized structures with no spatial growth this is an unwanted behaviour. For simulations aimed for studying objects with specific mass and thus specific redshift when they collapse, it is possible to specify a redshift after which the softening length is kept constant in physical units, but for simulations exploring a range of different objects this is not possible.
...
%TODO ei mitään inital conditioneista, onko tämä paha?
