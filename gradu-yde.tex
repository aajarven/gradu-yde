...
\subsection{Hubble Flow Measurements on Clusters}
...
The boxplots show some differences between the categories, but determining whether the differences arise from the characteristics of the samples or they are just effects of random sampling. A conclusion can be derived by applying the statistical testing methods introduced in section~\ref{sect:statistical_tests}. As assuming that the $H_0$ and zero point values are normally distributed is not justified, a test that can be used with two arbitrarily distributed data sets is an optimal choice. Thus the Kolmogorov--Smirnov test, described in section~\ref{sect:ks} is applied. The test is applied to pairs of categories, and a null hypothesis ``these two data sets are drawn from the same distribution'' is used with significance level $\alpha$ of 0.01. All other categories are tested against the results obtained using all haloes, in addition to which the two clustered categories with and without the mass criterion are tested against each other.
