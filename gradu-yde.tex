...
\reversemarginpar %TODO
\marginnote{\footnotesize{TODO: oispa parempi otsikko}}[1.5cm]
...
%TODO
\reversemarginpar
\marginnote{\footnotesize{TODO: mietippä pitäisikö seuraavat kappaleet siirtää ja jos niin minne vai pitäisikö ehkä poistaa kokonaan}}
...
Due to EDF being a result of random sampling, it may deviate from the underlying CDF considerably as can be seen by comparing CDFs in figure \ref{fig:cdf} and corresponding EDFs in figure \ref{fig:edf}. Latter figure also has another EDF corresponding to the green curve in the first figure to illustrate the differences that can arise from random sampling. This randomness also makes determining whether two samples are drawn from same distribution difficult. 
...
\begin{figure}
   ...
   \caption{Empirical distribution function for four random samples (N=35) drawn from same distributions as in figure \ref{fig:cdf}.}
   \label{fig:edf}
\end{figure}
...
%TODO otsikot ovat tällä haavaa yksi vitun sotku
